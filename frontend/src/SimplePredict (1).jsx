...
export default function SimplePredict({ onSwitchToClassic }) {
  ...
  useEffect(() => {
    if (!sportsError) return;
    const id = setTimeout(() => setSportsError(""), 5000);
    return () => clearTimeout(id);
  }, [sportsError]);
...
.sp-error{position:fixed;bottom:20px;left:50%;transform:translateX(-50%);max-width:600px;width:calc(100% - 40px);margin:0;padding:12px 18px;background:var(--red-dim);color:var(--red);border-radius:10px;font-size:13px;font-weight:600;box-shadow:0 8px 24px rgba(0,0,0,0.4);z-index:1000;border:1px solid var(--red);}
